...
def btn_execute_caesar_callback(sender, app_data, user_data):
    # user_data = { input: text_input, output: text_output, action: encrypt/decrypt }
...
with dpg.window(label="Chiffrement ROT13", width=200, pos=(500,300)):
    ...
    dpg.add_button(label="Apply", callback=btn_execute_caesar_callback, user_data="Some Data")
    # user data and callback set any time after button has been created
    btn = dpg.add_button(label="Apply 2", )
    dpg.set_item_callback(btn, btn_execute_caesar_callback)
...
with dpg.window(label="Chiffrement de César", autosize=True):
    ...
    with dpg.child_window(height=340,width=500, label='Chiffrer', no_scrollbar=True, no_scroll_with_mouse=True, show=True, tag='window_encrypt_caesar'):
        ...
        with dpg.group(horizontal=True):
            btn_encrypt_caesar = dpg.add_button(label="Chiffrer", callback=btn_execute_caesar_callback)
        ...
        dpg.set_item_user_data(item=btn_encrypt_caesar, user_data={'input': encrypt_text_input, 'output': encrypt_text_output, 'action': 'encrypt'})
    # Decrypt Caesar
    with dpg.child_window(height=340,width=500, label='Déchiffrer', no_scrollbar=True, no_scroll_with_mouse=True, show=False, tag='window_decrypt_caesar'):
        ...
        with dpg.group(horizontal=True):
            btn_decrypt_caesar = dpg.add_button(label="Déchiffer", user_data='text', callback=btn_execute_caesar_callback)
        ...
        dpg.set_item_user_data(item=btn_decrypt_caesar, user_data={'input': decrypt_text_input, 'output': decrypt_text_output, 'action': 'decrypt'})
    # Set local theme
